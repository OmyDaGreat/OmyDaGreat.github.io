...
import xyz.malefic.components.widgets.IconButton
import xyz.malefic.toSitePalette

val NavHeaderStyle =
  CssStyle.base {
    Modifier.background(rgba(255, 255, 255, 0f))
      .fillMaxWidth()
      .padding(1.cssRem)
      .backdropFilter(blur(4.px))
      .styleModifier { property("-webkit-backdrop-filter", "blur( 4px )") }
  }
...
// Note: When the user closes the side menu, we don't immediately stop rendering it (at which point
// it would disappear
// abruptly). Instead, we start animating it out and only stop rendering it when the animation is
// complete.
        ...
        Modifier.fillMaxHeight()
          ...
          .animation(
            SideMenuSlideInAnim.toAnimation(
              duration = 200.ms,
              timingFunction =
                if (menuState == SideMenuState.OPEN) AnimationTimingFunction.EaseOut
                else AnimationTimingFunction.EaseIn,
              direction =
                if (menuState == SideMenuState.OPEN) AnimationDirection.Normal
